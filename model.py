...
from sklearn.metrics import confusion_matrix, mean_squared_error, root_mean_squared_error, mean_absolute_error, \
    log_loss, accuracy_score, ConfusionMatrixDisplay
...
def save_models_kfold(indep, dep,
    ...
    for i, (train_index, test_index) in enumerate(kfold.split(indep, dep)):
        ...
        if balancing2:
            path = "Models/" + type(model).__name__ + model_appendix + pca_name + '_' + type(
                balancing).__name__ + balancing_appendix + '_' + type(
                balancing2).__name__ + balancing2_appendix + "_val_" + str(i) + ".pkl"
        else:
            path = "Models/" + type(model).__name__ + model_appendix + pca_name + '_' + type(
                balancing).__name__ + balancing_appendix + "_val_" + str(i) + ".pkl"
...
def save_models(indep_train, dep_train,
                model, model_appendix='',
                pca_dim=0,
                balancing=None, balancing_appendix='',
                balancing2=None, balancing2_appendix=''):
    ...
    if balancing2:
        path = "Models/Final/" + type(model).__name__ + model_appendix + pca_name + '_' + type(
            balancing).__name__ + balancing_appendix + '_' + type(balancing2).__name__ + balancing2_appendix + ".pkl"
    else:
        path = "Models/Final/" + type(model).__name__ + model_appendix + pca_name + '_' + type(
            balancing).__name__ + balancing_appendix + ".pkl"
    if os.path.exists(path):
        print(path + ' already exists.')
    else:
        if pca_dim > 0:
            pca_path = 'Models/PCA/Final/PCA' + '.pkl'
            if os.path.exists(pca_path):
                pca = pk.load(open(pca_path, 'rb'))
                indep_train = pca.transform(indep_train)[:, :pca_dim]
            else:
...
def load_final_model(model_name, balancing_name=None):
    path = "Models/Final/" + model_name + '_' + balancing_name + ".pkl"
    if os.path.exists(path):
...
def test_classification(indep_test, dep_test, model_name, pca_dim=0, balancing_name='', file_appendix=''):
    if pca_dim > 0:
        pca_name = 'PCA' + str(pca_dim) + '_'
        long_model_name = model_name + '_PCA' + str(pca_dim)
    else:
        pca_name = ''
        long_model_name = model_name
    model = load_final_model(long_model_name, balancing_name)
    evaluation_path = "Models/Final/Evaluation/" + model_name + '_' + pca_name + balancing_name + file_appendix + ".csv"
    cm_norm_path = "Models/Final/Evaluation/Confusion Matrix/" + model_name + '_' + pca_name + balancing_name + file_appendix + ".png"
